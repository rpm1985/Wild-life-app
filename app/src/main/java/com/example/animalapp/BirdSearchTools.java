...
import java.util.List;
 ...
 class BirdSearchTools {
     static ArrayList<Animal> searchByBellyColour(ArrayList<Animal> animalList, List<String> bellyColours){
        ArrayList<Animal> resultList = new ArrayList<>();
        Log.d("STARTING", "THIS SEARCH BY BELLY COLOUR HAS STARTED "+ bellyColours);
        for (Animal animal :
                animalList) {
            ArrayList<String> colours = new ArrayList<>(Arrays.asList(animal.getBellyColour().split(";")));
        ...
        for (Animal animal :
            ...
            if (sizesList.size() > 2){
                if(((sizesList.get(0) >= animal.getMinBodyLengthCm() && sizesList.get(0) <= animal.getMaxBodyLengthCm())
                        ...
                        && (animal.getType().equalsIgnoreCase("Bird"))) {
                    resultList.add(animal);
                }
...
//        }
     ...
     static ArrayList<Animal> searchByWingColour(ArrayList<Animal> animalList, List<String> wingColours){
        ArrayList<Animal> resultList = new ArrayList<>();
        Log.d("STARTING", "THIS SEARCH BY WING COLOUR HAS STARTED "+ wingColours);
        for (Animal animal :
                animalList) {
            ArrayList<String> colours = new ArrayList<>(Arrays.asList(animal.getWingColour().split(";")));
